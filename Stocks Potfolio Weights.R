...
options(max.print = 1000000)                  #20th Step: (Optional) Maximise the layout of your data.  
...
# 1st Step:Downolad the stocks that you want. Use getSymbols().
...
# You can do it for all your data. 
View(AAPL) 
# 2nd Step: Get & Assign specific data to a variable.
# If you need a specific variable you should use one the functions : Op() , Hi(), Lo(), Cl(), Vo(), Ad(). 
# If you need multiple variables you should use one of the functions: HLC(), OHLCV()
# You should assign to a specific variable. Example op_variable <- Op(Symbol or Variable name) 
# You should do it for all your data.
Cl_UPS <- Cl(UPS)
...
# 4TH STEP (optional): 
##--- Download Splits and Dividents.---## 
# Note: Apply this step only if you want to get the splits and dividents seperately. 
# You can apply it for each stock. 
# 1st Step: Download the Stock's split data.
aapl_splits <-getSplits("AAPL")
# 2nd Step: Download the Stock's divident data. 
...
#Note: Sometimes you may be interested in the performance of a specific subperiod.
# You can use it for every stock and period you may want. 
...
#Note: when the return distribution is asymetric (skewed), it is usually used as an additional risk measures that focus on describing the potential losses. 
#These measures are: 
# a) The Semi-Deviation: The calculation of the variability of returns below the mean return. 
# b) VaR (Value-at-Risk): A statistic that wuantifies the extent of possible financial losses over a specific time frame. 
# c) Expected Shortfall: A risk measure sensitive to the shape of the tail of the distribution of returns on a portfolio
# ! Note: Expected shortfaill at % level, is the expected return on the portfolio in the worst of cases. 
#------------------------------------------------------------------------------# 

# 10TH STEP: 
##--- Other Measures. ---##
# 1st Step:Calculate SemiDeviation. 
...
# How to interpret VaR: What is the largest loss I could potentially take within the next quarter such that I only have 5 / 2.5% probability of observing an evern larger loss. 
...
# Depending on what strategy and goal you want to accomplish choose one of the following startegies.
#------------------------------------------------------------------------------#

# 13TH STEP: 
# MANDATORY Step: Specify the retuns of the stock that you will use. 
returns <- merge(r.AAPL,r.ABBV,r.MA,r.NFLX,r.PYPL,r.UPS,r.ΙΒΜ) 
...
# 20TH STEP: 
## --- Add objectives. --- ## 
# Note: It is important to add each objective seperately and stored in the objectives slot in the portfolio specification object. 
...
# 3rd Step: Add a risk budjet objective. 
...
# 22TH STEP: 
## --- Optimize with periodic rebalancing. ---##
# Note: By using a periodic rebalancing (backtesting) and analyzing sample results you can have a better understanding and potentially refine the constraints and objectives. 
# ---------------------------------------------------#
# INFO:
# Search Size: How many portfolios to rest. By default is 20000.  
# Rebalance on: character string of period to rebalance on (days, weeks, months, quarters, years)
...
# Note: You can define the optimize method, search size, rebalance on, training period and rolling window. 
...
# 23TH STEP: 
## --- Refine constraints and objectives and optimize with periodic rebalancing. ---## 
# 1st Step: Create a portfolio specification. 
...
# 7th Step: Add a risk budget Objecrtive,  Minimum risk = 5% & Maximum risk = 10%.   
...
# 2nd Step: Compute Annualised Returns. 
